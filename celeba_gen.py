...
for i in range(int(args.training_size/1000)):
    os.system(f'cp -r ../subset_5000/subset_5000/00{i}* ../debug_train_{args.training_size}/debug_train_{args.training_size}/')
...
for i in range(int(args.training_size/1000)):
    os.system(f'cp -r ../subset_15k20k/subset_15k20k/01{5+i}* ../debug_test_{args.training_size}/debug_test_{args.training_size}/')
...
np.save(f'celebA_representation/AISTATS_betavae_repres_synth_{alias}',synth_representation)
...
np.save(f'celebA_representation/AISTATS_betavae_repres_real_{alias}', real_representation)
    
    
ref_dataset = ImageFolder(f'../debug_ref',
                       transform=tf.Compose([
                           tf.Resize(image_size),
                           tf.CenterCrop(image_size),
                           tf.ToTensor(),
...
test_dataset = ImageFolder(f'../debug_test_{args.training_size}',
                       transform=tf.Compose([
                           tf.Resize(image_size),
                           tf.CenterCrop(image_size),
                           tf.ToTensor(),
...
np.save(f'celebA_representation/AISTATS_betavae_repres_test_{alias}', test_representation)
